...
Codex acts as the **coordinator agent** for the Alhazen notebook OS. The OS has 6 layers, all backed by TypeDB as the single source of truth:
...
Agents are defined in `agents/` and resolved to `.Codex/agents/` via `agents-registry.yaml`:
...
> **Note:** Sub-agent dispatch via `Agent()` is not currently implemented. Agents are used as persona prompts loaded via Codex's `/agents` feature. Sub-agent orchestration may be added in the future.
...
> **For agents (Codex, Codex Desktop, worktrees):** Before doing any work, check whether the project has been built. If `local_skills/` does not exist, run `make build` from the project root. This installs Python dependencies, clones external skills, wires `.Codex/skills/` symlinks, and starts TypeDB. The build is idempotent — safe to re-run.
>
> ```bash
...
uv run python .Codex/skills/typedb-notebook/typedb_notebook.py insert-collection --name "smoke-test"
```
...
make build-skills     # Resolve skills-registry.yaml → local_skills/ + wire .Codex/skills/
make build-agents     # Resolve agents-registry.yaml → .Codex/agents/
...
**Phase 1 — Build (local dev, Codex):**
```bash
make build            # Full build: deps + skills + TypeDB
make build-env        # Install Python dependencies only
make build-skills     # Resolve skills registry → local_skills/ + wire .Codex/skills/
...
> implementation (run with Claude Code) to keep the approach undiluted and avoid the token-cost
> of a persistent service through the API. These targets remain but are unmaintained.
```bash
...
uv run python .Codex/skills/typedb-notebook/typedb_notebook.py export-db --database dismech

# Restore (drops and recreates the database)
uv run python .Codex/skills/typedb-notebook/typedb_notebook.py import-db \
...
└── information-content-entity (abstract) — content, format, cache-path
    ...
    └── note                           — Codex's analysis or annotation
```
...
- Use a `read_strategy` field in the YAML frontmatter to tell Codex which sections to read for which tasks — not every section needs to be read every time.
...
**`make build-skills`** resolves the registry into `local_skills/` and wires `.Codex/skills/` symlinks.
...
4. Run `make build-skills` to wire it into Codex
...
**`make build-agents`** resolves the registry and symlinks agents to `.Codex/agents/`.
...
4. Run `make build-agents` to wire it into Codex
...
**Philosophy:** Use scripts to minimize token usage. Scripts handle heavy lifting (pagination, bulk operations, API calls, TypeDB transactions) while Codex orchestrates at a higher level.
...
# 2. Create .Codex/skills/semantic-scholar/
...
**CLI usage:**
```bash
uv run python .Codex/skills/typedb-notebook/typedb_notebook.py insert-collection --name "Test"
uv run python .Codex/skills/scientific-literature/scientific_literature.py count --query "CRISPR"
uv run python .Codex/skills/jobhunt/jobhunt.py list-pipeline
# Note: .Codex/skills/* are symlinks → local_skills/* → skills/* (for core)
```
...
.Codex/skills/         # Gitignored — symlinks generated by make build-skills
├── typedb-notebook  -> ../../local_skills/typedb-notebook
└── jobhunt          -> ../../local_skills/jobhunt

.Codex/agents/         # Gitignored — symlinks generated by make build-agents
...
> **⚠️ DEPRECATED — OpenClaw deploy (Phase 2 / options B & C).** No longer supported. The project
> pivoted to a full Claude-based implementation: run interactively with Claude Code. This keeps the
> approach undiluted and avoids the token-cost overhead of a persistent OpenClaw service through the
> API. Phase 1 (Build) is the supported path; the material below is retained for reference only and
> is unmaintained.

**(Phase 1 — Build)** `make build` — local dev with Codex. Python deps + skills resolved + TypeDB ready.
...
# Deploy to VPS
cd deploy
./deploy.sh -t 5.78.187.158 -p anthropic -m Codex-sonnet-4-6 -k "$KEY"

# Deploy to Mac Mini
./deploy.sh -t 10.0.110.100 --target-type macmini -p anthropic -m Codex-sonnet-4-6 -k "$KEY"
```
...
- **Model IDs:** Use exact Anthropic model IDs (`Codex-sonnet-4-6`, `Codex-opus-4-6`, `Codex-haiku-4-5-20251001`). Incorrect IDs return HTTP 404.
...
1. **Detect** — Schema gap found (by agent or manual review)
...
When Codex makes a mistake, add it to this section so it doesn't happen again.
...
A **schema gap** is when Codex tries to represent a concept, relationship, or entity type that has no place in the current TypeDB schema. Schema gaps are the primary signal for knowledge graph evolution — they reveal what the schema needs to grow to support.

**Detection:** during sensemaking or planning you realize a concept, relationship, or entity type can't be stored (often surfaced by a TypeDB error code like `[SYR1]`, `[TYR01]`, `[FEX1]` in a skill's output). File immediately — don't wait until after the session.

**File a schema gap** as a GitHub issue (the `curation-skill-builder` gap-review workflow triages `gap:open` issues):
```bash
gh issue create \
  --repo <repo> \
...
```bash
uv run python .Codex/skills/curation-skill-builder/skill_builder.py \
...
- **Canonical inspection one-liner:**
  ```bash
  uv run python .Codex/skills/<skill>/<skill>.py <command> [args] 2>/dev/null \
